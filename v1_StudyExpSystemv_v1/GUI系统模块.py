...
from PyQt5.QtCore import (Qt, QTimer, QPropertyAnimation, QEasingCurve, QPoint, QParallelAnimationGroup)
...
class ExperienceWindow(QMainWindow):
    def __init__(self, exp_system):
        ...
        self.last_level = exp_system.user["current_level"]  # 存储上一次的等级
        self.last_segment = self.get_segment(exp_system.user["current_level"])  # 新增初始化段位追踪
    ...
    def update_data(self):
        ...
        # 检测升级
        # 修复后的检测逻辑（新增调试输出）
        print(f"[DEBUG] 当前等级: {current_level}, 上次等级: {self.last_level}")
        if current_level > self.last_level:
            print(f"触发升级动画：{self.last_level} → {current_level}")
            self.show_up_message(1)
            self.last_level = current_level # 更新等级

        # 检测升段
        if current_segment != self.last_segment:
            print(f"触发段位升级动画：{self.last_segment} → {current_segment}")
            self.show_up_message(2)
            self.last_segment = current_segment # 更新段位

    def show_up_message(self, chose:int):
        print("检测到升级或升段！")
        # # 创建消息弹窗
        # msg_box = QMessageBox()
        ...
        # """)
        # if chose == 1:
        #     msg_box.setText(f"✨ level up！✨")
        # elif chose == 2:
        #     msg_box.setText(f"✨ rank up！✨")

        # # 移除所有按钮
        # msg_box.setStandardButtons(QMessageBox.NoButton)
        
        # # 设置弹窗位置（屏幕右下角）
        # msg_box.show()  # 必须先显示才能获取正确尺寸
        # screen_geo = self.screen().availableGeometry()
        # msg_box.move(
        #     screen_geo.right() - msg_box.width() - 20,  # 右侧留20px边距
        #     screen_geo.bottom() - msg_box.height() - 20  # 底部留20px边距
        # )
